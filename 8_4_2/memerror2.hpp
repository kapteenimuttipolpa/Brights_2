...
// This is pretty much a poor man's std::array (with some
// not-immediately-obvious memory problems)
template <typename T, std::size_t Count> class flawed_array {
public:
  using value_type = T;
  using reference = T &;
  ...
  constexpr reference operator[](const std::size_t index) noexcept {
    return reinterpret_cast<reference>(
        *(data_storage + index * sizeof(value_type)));
  }
  ...
  constexpr const_iterator end() const noexcept {
    return reinterpret_cast<const_pointer>(data_storage +
